...
const resetPlan = (i) => {
  message.confirm({
    ...
    const res = await deleteRecord(i.maintenanceRecordId)
    if(res.code == 200) {
      queryParams.pageNum = 1
      getData()
      uni.showToast({
...
<template>
  <view class="warning-page">
    ...
    <view class="list-con" v-else>
      ...
      <view class="list-item" v-for="i in list" :key="i.maintenanceRecordId" @click.stop="clickToDetail(i.maintenanceRecordId)">
        <wd-card>
          <template #title>
            <view style="display: flex; justify-content: space-between; align-items: center">
              ...
              <view class="right">
                ...
                <view class="value">{{ i.maintenanceRecordTypeName }}</view>
              </view>
            </view>
          </template>
          <view class="main">
            <view class="content">
              <view class="content-item pd">
                <view class="label">保养日期：</view>
                <view class="value">{{ i.maintenanceDate }}</view>
              </view>
              <view class="content-item pd">
                <view class="label">有效日期（月）：</view>
                <view class="value">{{ i.maintenanceValidDate || '无' }}</view>
              </view>
              <view class="content-item pd">
                <view class="label">保养内容：</view>
                <view class="value" style="width: 60%;">
                  <wd-text :text="i.maintenanceContent" :lines="2" color="#333"></wd-text>
                </view>
              </view>
            </view>
